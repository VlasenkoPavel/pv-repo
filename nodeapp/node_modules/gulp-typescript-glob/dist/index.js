...
exports.default = gulpTypescriptGlob;
...
function transform(file, env, callback) {
    ...
    var reg = /import\s+["'](((\.{1,2}\/)*)[^"']*?\*(\.ts|\.tsx)?)["'];?/;
    var base = _path2.default.join(_path2.default.normalize(_path2.default.dirname(file.history[0])), '/');
    ...
    var _loop = function _loop() {
        ...
        files.forEach(function (filename) {
            if (filename !== file.path && isTypescript(filename)) {
                ...
                var basePath = _path2.default.join(base, trailers);

                filename = _path2.default.normalize(filename).replace(basePath, '');

                filename = filename.replace(/\.[^/.]+$/, "");

                var importItem = 'import "' + trailers + (0, _slash2.default)(filename) + '";';
...
function isTypescript(filename) {
    ...
    return !_fs2.default.statSync(filename).isDirectory() && _path2.default.extname(filename).match(/\.ts|\.tsx/i);
}
